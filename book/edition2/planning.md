...
Why update the mean and variance to the elites' moments? Fix one action coordinate and write $A_i$ for its value in elite sequence $i$. Let $n_e=|\mathcal E|$ be the number of elites. We fit these selected values as if they were independent samples from $\mathcal N(\mu,\sigma^2)$.
...
$$\mathcal L(\mu,\sigma)=\prod_{i\in\mathcal E}\frac1{\sqrt{2\pi}\sigma}\exp\!\left[-\frac{(A_i-\mu)^2}{2\sigma^2}\right],\qquad\sigma>0.$$
...
$$-\log\mathcal L=\frac{n_e}{2}\log(2\pi)+n_e\log\sigma+\frac1{2\sigma^2}\sum_{i\in\mathcal E}(A_i-\mu)^2.$$

The first term is constant with respect to the fitted parameters. The mean derivative is $\sum_i(\mu-A_i)/\sigma^2$, so its zero gives $\mu_*=(1/n_e)\sum_iA_i$. Its positive second derivative $n_e/\sigma^2$ makes this the unique minimum for any fixed positive variance.

Now write $v=\sigma^2$ and $S=\sum_i(A_i-\mu_*)^2$. The remaining variance-dependent terms are $(n_e/2)\log v+S/(2v)$. Their derivative is $(n_ev-S)/(2v^2)$. For $S>0$, it is negative below $S/n_e$ and positive above it, proving

$$v_* = \frac1{n_e}\sum_{i\in\mathcal E}(A_i-\mu_*)^2.$$
