...
The header's **Listen** control starts continuous narration at the current passage. Double-click a passage, or select text and use the small **Listen / Explain** menu, to focus on one part. Figures, tables, code blocks, and labs have their own actions. The compact header player provides pause, next, previous, seeking, and a Follow toggle. Its generated audio is cached locally; 93 teaching figures, all 15 tables, and all 133 display equations have authored or reviewed spoken explanations. The course index covers every source equation, including inline math, and lets the tutor refer to an exact visible passage.

**Assistant** starts a GPT-Live 1 voice conversation. GPT-6 Sol is the default reasoning model; settings also offer GPT-6 Luna, GPT-6 Astra, and GPT-5.6 Terra. You can change it during a conversation. For a technical question, one course action locates the exact passage, scrolls to it, keeps it highlighted, and returns teaching notes for the explanation. This includes the paper's numbered equations. A destination stays pinned during its answer; related lookups do not move the page. Scrolling yourself cancels pending page movement. Figures retain their original colors, with their title or caption highlighted. The assistant can also move a validated widget control and start a prepared ElevenLabs narration. You can type a question in the same panel. The header keeps microphone mute, assistant-sound mute, pause, and End available without opening that panel.

Open the settings icon beside Print and enter your own ElevenLabs and OpenAI API keys. The site sends them directly from your browser to those providers; they are never built into the HTML or committed to Git. By default, keys stay in this browser tab's session storage. **Remember keys on this device** explicitly opts into local storage; **Clear keys** removes them. The local `.env` is Git-ignored and used only for development tests. During narration, the assistant's microphone and sound are muted, and the current passage ID is retained locally so a paused reader can ask about it. Muting and pausing do not end a paid GPT-Live session; **End** closes it. Narration and live assistance require internet access and provider billing; the rest of the course remains usable without them. GPT-Live 1 voice requires HTTPS or localhost and microphone access. The public GitHub Pages edition is a bring-your-own-key static site, so its browser-side key handling differs from OpenAI's recommended trusted-server session setup. Use a project key with spending limits that suit you.
...
- `npm run verify:voice`: narration-index coverage and browser binding, then voice-interface layout, settings, selection, exact-equation focus, and course-tool checks without provider calls. `node tools/verify-live-model-switch.mjs` checks live model-update acknowledgment handling. The live provider handshake, model switch, course focus, and ElevenLabs playback have also been checked manually with local, uncommitted keys.
- `npm run verify:voice:navigation`: production transport/UI event replay, 80 topic and missing-material cases, competing and stale requests, mobile destinations, and pixel-identical SVGs before/after highlighting. This also runs inside `verify:voice` and gates GitHub Pages deployments. See the [navigation contract and harness](research/voice/NAVIGATION.md).
