...
Two pictures can help. If velocity is constant between frames separated by time $\Delta t$, then $q_t-q_{t-1}=v\Delta t$. Dividing both sides gives $v=(q_t-q_{t-1})/\Delta t$. If acceleration or occlusion matters, more history or a different state estimator may be needed. History is evidence about hidden state, not a magic guarantee of full observability.

A state description is **Markov** for a prediction problem when, given that state and the relevant action, earlier history supplies no additional information about the next state. This is a conditional-independence property; it does not mean the world has no memory. The state must already include whatever memory matters. A compact learned embedding may fail to have that property even if the physical simulator's full state has it.
...
<figure class="plate"><img src="assets/generated/abstraction.png" alt="The same robot arm appears as a textured camera scene, a simplified violet configuration, and teal possible future poses."/><figcaption>Abstraction retains selected distinctions. This generated illustration is an explanatory metaphor: the model's learned vector is not a hand-drawn arm, and imagined poses are not measured predictions.</figcaption></figure>
...
<figure class="plate"><img src="assets/generated/hierarchy.png" alt="A robot plans a route across rooms, reaches intermediate subgoals, and controls individual arm movements."/><figcaption>Different time scales invite different descriptions. This generated conceptual plate illustrates hierarchical planning; the browser laboratory and LeWM do not implement this full hierarchy.</figcaption></figure>
