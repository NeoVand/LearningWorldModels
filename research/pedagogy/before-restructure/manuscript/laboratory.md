...
The planner receives two camera images, the previous action, and a goal image. It samples candidate action sequences, rolls them forward through the learned predictor, and scores their distance from the encoded goal plus an action-effort term. It refines the candidates using the cross-entropy method, executes the first action, and observes again.
