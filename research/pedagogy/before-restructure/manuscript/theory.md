...
<p class="lead">A useful theorem is a statement together with its assumptions. This chapter reconstructs the theoretical motivation for isotropy and Gaussianity, then draws a boundary around what it establishes.</p>
...
$$\mathbb E\big[\Delta m+2\nabla m^\top s\big]^2\leq2B_0^2+8L^2J(p).$$
...
This is research-level reading: neither dismiss the theory nor let its shorthand outrun its proof. We can now implement the world-model objective while keeping its mathematical and empirical contracts separate.
