...
Consider $\hat y=wx+b$. The input $x$ changes from example to example. The parameters $w,b$ are shared across examples and adjusted during training. Given target $y$, choose the loss $\ell=\tfrac12(\hat y-y)^2$. The factor $1/2$ is a convenient scaling convention; it cancels the 2 from differentiating a square.
...
## Momentum and Adam are choices, not theorems of intelligence

Momentum averages recent gradients so a persistent direction accumulates while some fluctuations cancel. One convention is $m_t=\beta m_{t-1}+(1-\beta)g_t$, initialized at zero. Expanding the recurrence gives $m_t=(1-\beta)\sum_{k=1}^t\beta^{t-k}g_k$. If every gradient equals a fixed $g$, this becomes $(1-\beta^t)g$ by the finite geometric-series formula. Dividing by $1-\beta^t$ removes the zero-initialization bias under that stationary-mean model.
...
For vector weights and a design matrix $X$, the same expansion yields the normal equations
...
## Why weight decay does not solve JEPA collapse

A constant zero encoder often has small or zero weights. Penalizing those weights can make collapse more attractive, not less. SIGReg instead penalizes the distribution of outputs over a batch. It asks a collection of embeddings to exhibit a specified geometry, while prediction asks temporally related embeddings to be compatible.
