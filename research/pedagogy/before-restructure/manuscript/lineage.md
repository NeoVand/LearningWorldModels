...
The target encoder is updated by an exponential moving average, and the target branch is detached from gradient computation. Thus I-JEPA belongs to the teacher-based family described in the previous chapter. It is not the same symmetric end-to-end objective as LeWM.
...
Before entering SIGReg, we need one architectural bridge. The final paper's encoder and predictor are transformers. We can understand their arithmetic using the same dot products, weighted averages, and chain rules already developed.
