...
LeWM's visualization decoder uses the encoder's CLS representation and learned patch queries with cross-attention. In cross-attention, queries come from one collection while keys and values come from another; the same weighted-average formula applies. The decoder's training signal is reconstruction, but it is not fed back into the main world-model training in the default experiment.
...
You now have the tools to read every family of evaluation in the final paper. The capstone asks you to assemble the entire argument yourself, from a data window to a qualified scientific conclusion.
