...
function render(body, id) {
  body = body.replace("<!-- LEARNING_ROUTE -->", "| Stage | Read | Check before moving on |\n|---|---|---|\n" + chapters.filter(c => c[0] !== "reference").map(([key, title, stage, , outcome]) => `| ${stage} | [${title}](#${key}) | ${outcome} |`).join("\n"));
  body = body.replace(
    /<!-- CODE: ([^ ]+) -->/g,
    (_, p) =>
      "```" +
  ...
  html = html.replace(/<details(?: class="[^"]*")?><summary>Required[^<]*?· ([^<]+)<\/summary>([\s\S]*?)<\/details>/g,
    (_, title, body) => `<section class="proof"><h3>Derivation · ${title}</h3>${body}</section>`);
...
const content = chapters
  .map(
    ([id, title, status, prerequisites, outcome], i) =>
      `<article id="${id}"><div class="chapter-meta"><span>${String(i + 1).padStart(2, "0")} / ${status}</span><span>World models, from first principles</span></div>${render(read(`book/edition2/${id}.md`), id)}<div class="chapter-check"><span class="eyebrow">Before moving on</span><p>${outcome}</p>${i + 1 < chapters.length ? `<a href="#${chapters[i + 1][0]}">Continue · ${chapters[i + 1][1]} →</a>` : ""}</div></article>`,
  )
  .join("\n");
let js = "";
