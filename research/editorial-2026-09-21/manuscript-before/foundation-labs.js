...
const palette={ink:'#394549',line:'#dce1da',obs:'#245ca6',lat:'#7044ad',pred:'#13796f',act:'#986009',loss:'#b34832'};
function desk(id,inputId,render){const canvas=document.getElementById(id),input=document.getElementById(inputId);if(!canvas||!input)return;
 ...
 input.addEventListener('input',draw);new ResizeObserver(draw).observe(canvas);document.fonts.ready.then(draw);draw();}
...
desk('normality-canvas','normality-size',(c,w,h,exponent)=>{
 ...
 counts.forEach((n,i)=>{const height=n/peak*(bottom-top);c.fillStyle='#7044ad45';c.fillRect(left+i*dx+1,bottom-height,Math.max(1,dx-2),height);});
