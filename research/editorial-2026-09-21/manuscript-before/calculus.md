...
First, such a direction exists. The unit sphere is closed and bounded in finite-dimensional Euclidean space. Any sequence on it has a convergent subsequence with a limit on that sphere. Choose a sequence whose quadratic values approach their supremum. Continuity makes the limit's value equal that supremum. This uses the basic completeness property of real coordinates; it is the finite-dimensional extreme-value argument, not an assumption about the data.
