...
<p class="instruction">First inspect the untrained model. Then train for 5,000 updates, evaluate, and run the matched prediction-only comparison.</p>
<div class="controls"><label>Seed <input type="number" id="world-seed" value="17" min="1" max="99999"/></label><button class="primary" id="world-init">Prepare model</button><label>Updates <select id="world-budget"><option value="500">500</option><option value="2000">2,000</option><option value="5000" selected>5,000</option></select></label><button id="world-train" disabled>Train</button><button id="world-stop" disabled>Pause</button></div>
<div id="world-status" class="state" role="status">Ready to generate images and initialize random weights.</div>
...
**Prediction / persistence** compares the trained predictor's squared error with copying the current embedding, evaluated in the same learned representation. A ratio below one means that this predictor improves on that particular baseline. It does not by itself prove that the representation preserves useful physical information.
...
Do the prediction-only experiment even if the regularized model looks successful. It demonstrates why the objective contains two terms. A lower prediction loss is not automatically the better result when the encoder can alter the scale and content of its targets.

## Let the learned model choose actions

The planner receives two camera images, the previous action, and a goal image. It samples candidate action sequences, rolls them forward through the learned predictor, and scores their distance from the encoded goal plus an action-effort term. It keeps promising candidates, resamples around them, executes the first action, and observes again. This is the sample–score–refit procedure called the cross-entropy method. The next chapter derives the procedure; here you can observe the basic loop without treating its name as an explanation.
...
<div class="controls"><label>Goal <select id="world-goal"><option value="0">A · local reach</option><option value="1">B · local turn</option><option value="2">C · local curl</option><option value="3">D · distant stress test</option></select></label><button id="world-control-reset" disabled>Reset mechanism</button><button id="world-control" disabled>Run 20 actions</button></div>
...
<canvas id="world-control-plot" class="wide-chart" width="900" height="450" aria-label="Actual arm, goal arm, and imagined future poses"></canvas>
...
$$\loss=\frac{1}{Bd}\sum_{b=1}^B\sum_{j=1}^d(\pred_{b,j}-\lat_{b,\mathrm{next},j})^2+\lambda\frac{1}{3}\sum_{r=1}^3\reg(\Z_r),\qquad\lambda=0.01.$$
...
On this held-out batch, the predictor performs better with the supplied action alignment than with the chosen shuffle. This supports action sensitivity in the tested distribution. It does not establish accurate responses to all counterfactual actions, especially those absent from the exploration corpus. A stronger investigation changes exploration coverage and tests deliberately chosen interventions.
