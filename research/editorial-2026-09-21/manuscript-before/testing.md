...
The supremum means the smallest upper bound over every real $x$; it includes values approached immediately before a jump. This is the Kolmogorov–Smirnov discrepancy for a fully specified standard-Gaussian reference. We use it because its geometry is easy to inspect, not because SIGReg optimizes this particular statistic.

Sort the samples as $x_{(1)}\leq\cdots\leq x_{(n)}$. Immediately before the $i$th jump the empirical fraction is $(i-1)/n$; immediately after it is $i/n$. Between jumps the empirical curve is flat while $\Phi$ is monotone. The largest gap therefore occurs at one of those sides, giving the finite calculation
...
Furthermore, $D_n$ involves sorting, a maximum, and indicator-based cumulative curves. We want a smooth discrepancy that supplies useful gradients to every embedding coordinate. SIGReg will replace this illustrative statistic with characteristic-function measurements, then average their squared deviations. The ideas of finite-sample fluctuation and a fully specified target still apply.
